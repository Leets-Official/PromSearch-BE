...
class TagControllerTest {
    ...
    @DisplayName("태그 목록은 타입별 기존 태그를 조회한다")
    @Test
    void listTags() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/tags").param("type", "JOB"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.result[0].tagId").value(1))
                .andExpect(jsonPath("$.result[0].tagType").value("JOB"))
                .andExpect(jsonPath("$.result[0].name").value("학생"))
                .andExpect(jsonPath("$.result[1].name").value("직장인"));
    }

    @DisplayName("태그 목록은 지원하지 않는 타입 문자열을 400으로 거절한다")
    @Test
    void listTagsRejectsInvalidType() throws Exception {
        mockMvc.perform(get("/api/v1/tags").param("type", "UNKNOWN"))
